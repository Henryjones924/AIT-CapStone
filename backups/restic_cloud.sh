...
# Setting up Logging
LOG_FILE="/var/log/restic/restic_cloud.log"
...
restic forget --keep-daily 7 --keep-weekly 5 --keep-monthly 6 --prune -c >> >(while IFS= read -r line; do log "INFO" "$line"; done) 2> >(while IFS= read -r line; do log "ERROR" "$line"; done)
sleep 2
